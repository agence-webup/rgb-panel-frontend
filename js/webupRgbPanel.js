            ...
            */
            ...
            function drawOnCanvas(x, y, color) {
                ...
                if (thickness == 1) {
                    var ctx = canvas.getContext('2d')
                    ctx.imageSmoothingEnabled = false
                    ctx.fillStyle = (matrix[pixelX][pixelY] == "#000" ? '#BABABA' : matrix[pixelX][pixelY])
                ...
                for(i=0; i < matrix.length; i++) {
                    ...
                    for(j=0; j < matrix[i].length; j++) {
                        ...
                        if (matrix[i][j] != '#000') {
                            ...
                            drawing.push({ // using the + operand in order to make sure integers are being passed into the object
                                "x": +i,
                                "y": +j,
                                "r": +rgb.r,
                                "g": +rgb.g,
