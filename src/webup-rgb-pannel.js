export default class WebupRgbPannel {
   ...
   */
    ...
    this.canvas.addEventListener('mousemove', e => {
      if (this.mouseDown == true) {
        const canvasWidth = panel.getBoundingClientRect().width
    ...
    this.canvas.addEventListener('touchmove', e => {
      if (tihs.mouseDown == true) {
        e.preventDefault()
        const canvasWidth = this.panel.getBoundingClientRect().width
        const imageWidth = this.matrix.length * 13

        const ratio = imageWidth / canvasWidth

        const mouseX = (e.pageX - canvas.offsetLeft) * ratio
        const mouseY = (e.pageY - canvas.offsetTop) * ratio
    ...
    this.thicknessControl.addEventListener('change', function () {
      const chosenThickness = this.thicknessControl.value
